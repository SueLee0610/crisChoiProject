...
public class Board extends JPanel implements ActionListener, Runnable { //Panel 위에서 돌아가므로 Panel을 상속해야 
    ...
    public static int v = 172; //점프할 때 바뀌는 y값 변수
    ...
    public Board(String backgroundImageLocation, List<Enemy> enemies, GameStateListener listener, Character character) {
        ...
        public void paint(Graphics g) {
        	...
        	if (character.getYMove() == 1 && done2 == false) { //run메소드를 한 번만 할 수 있게끔 함(스레드), 그러나 더블점프할 경우 done2 불리언 사용 ㄴㄴ
                ...
                g2d.drawImage(character.getImage(), character.getLeft(), v, null); //(캐릭터, x시작점, y시작점, observer)
 
                
                if (character.getXMove() == -1) {
                        g2d.drawImage(imgBackground, 685 - character.getnX2(), 0, null);
                        g2d.drawImage(character.getImage(), character.getLeft(), v, null); //캐릭터가 왼쪽으로 갈 때
                }
                ...
                g2d.drawString("남은 총알 수: " + character.getAmmo(), 500, 40);
